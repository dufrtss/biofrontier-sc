...
function Section({ id, title, children }: SectionProps) {
  return (
    <section id={`methodology-${id}`} className="scroll-mt-4">
      <h3 className="text-sm font-semibold text-[--text-primary] uppercase tracking-wider mb-3 pb-2 border-b border-[--border]/60">
        {title}
      </h3>
...
function Code({ children }: { children: string }) {
  return (
    <pre className="bg-[--surface-raised] border border-[--border] rounded px-3 py-2 font-mono text-[--accent] text-[11px] overflow-x-auto whitespace-pre-wrap">
      {children}
    </pre>
  )
}
...
export default function MethodologyPanel({ open, initialSection, onClose }: MethodologyPanelProps) {
  ...
  return (
    <>
      ...
      <div
        className={[
          'fixed right-0 top-0 h-full w-full md:w-[480px] bg-[--surface] border-l border-[--border] z-[2000] flex flex-col transition-transform duration-200',
